"""BoseFormer (Slater × BoseFormer) parent run script — MoTe2 three-orbital model.
...
-------------------------------
results/mote2_three_orbital/boseformer/
├── run.py                          ← this file (never modified by a run)
└── jobs/
    └── boseformer_scm_3_6_m3_3_nf9_V1_0.30_emb<tag>_sl<tag>_mc_YYYYMMDD_HHMMSS/
...
--------------------------------------
  ...
  NUM_LAYERS                  BoseFormer encoder depth (default: 4)
  ...
  SLATER_INIT_MODE            random | noninteracting (default: random)
                                random        — random Slater orbital init
                                noninteracting — warm-start from NI orbitals
...
_LAUNCH_ROOT = Path(__file__).resolve().parent.parent / "results" / "mote2_three_orbital" / "boseformer"
...
def main() -> None:
    ...
    # Physical system
    supercell_matrix = np.array([[2, 0], [0, 2]], dtype=np.int64)
    n_fermions = 4
    ...
    slater_init_mode  = _env_str("SLATER_INIT_MODE", "random")
    embedding_type    = _env_str("EMBEDDING_TYPE", "periodic")

    if slater_init_mode not in ("random", "noninteracting"):
        raise ValueError(f"SLATER_INIT_MODE must be 'random' or 'noninteracting', got {slater_init_mode!r}")
    ...
    ax.set(title="MoTe2 three-orbital: Slater × BoseFormer optimisation",
