...
def memory_poll_worker(stop_event, interval_s, series):
    ...
    while not stop_event.is_set():
        series.append(
            {
                "time_unix_s": float(time.time()),
                "process_gpu_memory_mib": query_process_gpu_memory_mib(pid=pid),
            }
        )
            ...
            writer.writerow(
                {
                    ...
                    "process_gpu_memory_mib": (
                        "" if row["process_gpu_memory_mib"] is None else row["process_gpu_memory_mib"]
                    ),
                }
            )
            ...
            writer.writerow(
                {
                    ...
                    "process_gpu_memory_mib": snap.get("process_gpu_memory_mib"),
...
def make_process_memory_timeline_plot(path, series, run_start_time, run_end_time):
    ...
    ys = []
    for row in series:
        if row["process_gpu_memory_mib"] is None:
            continue
        if run_start_time is None:
            continue
        xs.append(row["time_unix_s"] - run_start_time)
        ys.append(row["process_gpu_memory_mib"])
    ...
    ax.set_ylabel("Process GPU memory [MiB]")
...
def make_stage_memory_plot(path, snapshots):
    ...
    values = [
        np.nan if snap.get("process_gpu_memory_mib") is None else snap["process_gpu_memory_mib"]
        for snap in snapshots
    ]
...
def main():
    ...
    process_mibs = [
        float(snap["process_gpu_memory_mib"])
        for snap in snapshots
        if snap.get("process_gpu_memory_mib") is not None
    ]
    poll_mibs = [
        float(row["process_gpu_memory_mib"])
        for row in poll_series
        if row.get("process_gpu_memory_mib") is not None
    ]
    peak_snapshot_mib = max(process_mibs) if len(process_mibs) > 0 else None
    peak_polled_mib = max(poll_mibs) if len(poll_mibs) > 0 else None
    overall_peak_mib = None
    if peak_snapshot_mib is not None and peak_polled_mib is not None:
        overall_peak_mib = max(peak_snapshot_mib, peak_polled_mib)
    elif peak_snapshot_mib is not None:
        overall_peak_mib = peak_snapshot_mib
    elif peak_polled_mib is not None:
        overall_peak_mib = peak_polled_mib

    model_ready_mib = None
    after_run_mib = None
    for snap in snapshots:
        if snap["label"] == "after_driver_init":
            model_ready_mib = snap.get("process_gpu_memory_mib")
        if snap["label"] == "after_run":
            after_run_mib = snap.get("process_gpu_memory_mib")
    ...
    summary = {
        ...
        "process_gpu_memory_mib": {
            ...
            "after_run": after_run_mib,
            "peak_snapshot": peak_snapshot_mib,
            "peak_polled": peak_polled_mib,
            "overall_peak": overall_peak_mib,
            "delta_after_run_minus_model_ready": (
                None
                if model_ready_mib is None or after_run_mib is None
                else float(after_run_mib) - float(model_ready_mib)
            ),
            "delta_peak_minus_model_ready": (
                None
                if model_ready_mib is None or overall_peak_mib is None
                else float(overall_peak_mib) - float(model_ready_mib)
            ),
        },
        "device_memory_stats": {
    ...
    make_process_memory_timeline_plot(
        plots_dir / "process_memory_timeline_mib.png",
        poll_series,
        run_start_time,
        run_end_time,
    )
    make_stage_memory_plot(plots_dir / "process_memory_by_stage_mib.png", snapshots)
    ...
    print(
        "Process memory [MiB]: "
        f"model_ready={model_ready_mib}, "
        f"peak={overall_peak_mib}, "
        f"after_run={after_run_mib}"
    )
    print(
        "Parameter memory [MiB]: "
        f"total={param_bytes_total / (1024.0**2):.6f}, "
        f"slater={param_bytes_slater / (1024.0**2):.6f}, "
        f"boseformer={param_bytes_boseformer / (1024.0**2):.6f}"
    )
