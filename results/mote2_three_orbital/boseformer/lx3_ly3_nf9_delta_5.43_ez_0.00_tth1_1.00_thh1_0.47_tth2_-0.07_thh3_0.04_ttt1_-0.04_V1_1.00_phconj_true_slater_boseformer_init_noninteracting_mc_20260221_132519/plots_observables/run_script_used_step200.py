...
from aidan_custom.models import LogSlaterBoseFormer, make_supercell_reciprocal_vectors
...
def _minimum_image_translations(
    ...
    for n1 in (-1, 0, 1):
        for n2 in (-1, 0, 1):
            translations.append(
                float(n1) * np.asarray(supercell_t1, dtype=np.float64)
                + float(n2) * np.asarray(supercell_t2, dtype=np.float64)
            )
...
def _pair_correlation_from_samples(
    ...
    dist_matrix = np.full((n_sites, n_sites), np.inf, dtype=np.float64)
    for t in np.asarray(translations, dtype=np.float64):
        dist_matrix = np.minimum(
            dist_matrix,
            np.linalg.norm(disp + t[np.newaxis, np.newaxis, :], axis=-1),
        )
...
def _q_vectors_from_lattice(lattice, a_m: float) -> np.ndarray:
    # Written with Codex 02-21-26.
    ...
    for kx in range(int(lattice.Lx)):
        for ky in range(int(lattice.Ly)):
            ...
            q_vectors[idx] = coeff[0] * b1 + coeff[1] * b2
            idx += 1
    return q_vectors
...
def main():
    ...
    # BoseFormer controls.
    boseformer_num_layers = int(os.environ.get("BOSEFORMER_NUM_LAYERS", "2"))
    boseformer_d_model = int(os.environ.get("BOSEFORMER_D_MODEL", "32"))
    boseformer_n_heads = int(os.environ.get("BOSEFORMER_N_HEADS", "2"))
    boseformer_mlp_hidden_factor = int(
        os.environ.get("BOSEFORMER_MLP_HIDDEN_FACTOR", "2")
    )
    slater_init_mode = os.environ.get("SLATER_INIT_MODE", "noninteracting")
    ...
    if int(graph.n_nodes) != 27:
        raise RuntimeError(
            f"Expected a 27-site problem for this job, got n_sites={graph.n_nodes}."
        )
    ...
    (
        ...
        supercell_t2,
        lattice,
    ) = _mote2_site_positions_and_geometry(Lx=Lx, Ly=Ly, a_m=a_m)
    ...
    q_list = _q_vectors_from_lattice(lattice=lattice, a_m=a_m)
