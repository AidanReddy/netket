...
def _select_seed_params(df: pd.DataFrame, n_seeds: int, gap_floor: float) -> pd.DataFrame:
    ...
    out = strict.sort_values(["score", "flatness_ratio", "trace_violation"]).head(int(n_seeds))
...
def _polish_round(
    ...
    strict["score"] = _score(strict)
    top = strict.sort_values(["score", "flatness_ratio", "trace_violation"]).head(90)
...
def _plot_objective(df_all: pd.DataFrame, df_good: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Broadened Anchor Search (lowest band, ph_conj=True, nk=11)")
    ax.grid(alpha=0.25)
    ax.legend(loc="best")
    fig.colorbar(sc, ax=ax, label="Chern (lowest)")
    fig.tight_layout()
    fig.savefig(out_dir / "objective_scatter_q_minus_absC.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    print("Evaluating anchor baseline on nk=11 ...", flush=True)
    ...
    for seed in SEEDS:
        ...
        best_seed = combo.sort_values(["score", "flatness_ratio", "trace_violation"]).iloc[0]
    ...
    strict["score"] = _score(strict)
    strict = strict.sort_values(["score", "flatness_ratio", "trace_violation"]).reset_index(drop=True)

    best_by_flat = strict.sort_values(["flatness_ratio", "trace_violation", "chern_mag_error"]).head(1)
    best_by_trace = strict.sort_values(["trace_violation", "flatness_ratio", "chern_mag_error"]).head(1)
    ...
    selected = pd.concat([best_by_flat, best_by_trace, strict.head(4)], ignore_index=True)
    ...
    summary = {
        ...
        "search_type": "bestcheck_anchor_local_wider",
        "ph_conj": True,
        "band": "lowest",
    ...
    for name in [
        ...
        "objective_scatter_q_minus_absC.png",
